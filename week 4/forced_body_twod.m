function [Phi,Phi_n,refl,trans] = forced_body_twod(alpha,number_modes,...
                   panelsL,panelsR,panels0,panelsB,panelsF,option)
% [Phi,Phi_n,refl,trans] = forced_body_twod(alpha,number_modes,panelsL,panelsR,panels0,panelsB,panelsF,option)
% 
...
F_big=[zeros(NTOT-NF-NB,1);-F;zeros(NF,1)];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%solving
Phi=(1/2*eye(NL+NR+N0+NB+NF)-H+G*Q_big)\(G*F_big);
...
%%%%%%%%%%%%%%%%%%%%%%%%%% function QF_matrices %%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [Q,F,S,R,K]=QF_matrices(panelsL,alpha,number_modes)
       ...
       if(option == 'heave') 
         for k=1:NB,            
             F(k) = (panelsB(k,1)-panelsB(k,3))/...
                ((panelsB(k,3)-panelsB(k,1))^2+(panelsB(k,4)-panelsB(k,2))^2)^(1/2);
         end
       elseif(option == 'pitch')
         for k=1:NB,
             nx = -(panelsB(k,2)-panelsB(k,4))/...
                ((panelsB(k,3)-panelsB(k,1))^2+(panelsB(k,4)-panelsB(k,2))^2)^(1/2); 
             nz = (panelsB(k,1)-panelsB(k,3))/...
                ((panelsB(k,3)-panelsB(k,1))^2+(panelsB(k,4)-panelsB(k,2))^2)^(1/2);
             xp = (panelsB(k,1) + panelsB(k,3))/2;
             zp = (panelsB(k,2) + panelsB(k,4))/2;
            F(k) = zp*nx - xp*nz;
         end           
       end
